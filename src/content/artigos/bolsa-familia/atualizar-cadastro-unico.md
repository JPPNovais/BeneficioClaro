---
title: "Como atualizar o CadÚnico sem perder o benefício"
slug: "atualizar-cadastro-unico"
categoria: "bolsa-familia"
description: "Quando e como atualizar o Cadastro Único em 2026: o prazo de 24 meses, as mudanças que exigem atualização imediata, a averiguação cadastral e o que acontece se você não atualizar."
respostaRapida: "Você deve atualizar o CadÚnico sempre que mudar endereço, renda ou composição da família e, mesmo sem mudanças, pelo menos a cada 24 meses. A atualização é gratuita, feita no CRAS. Ficar mais de 2 anos sem atualizar pode bloquear ou cancelar o Bolsa Família e outros benefícios ligados ao cadastro."
dataPublicacao: 2026-06-22
dataAtualizacao: 2026-06-25
autor: "ana_ribeiro"
revisado: true
subtopico: "inscricao"
tags:
  - "cadúnico"
  - "atualização cadastral"
  - "averiguação cadastral"
  - "cras"
fontesOficiais:
  - label: "Cadastro Único — Ministério do Desenvolvimento (gov.br)"
    href: "https://www.gov.br/mds/pt-br/acoes-e-programas/cadastro-unico"
  - label: "Tire suas dúvidas sobre o novo Cadastro Único (gov.br)"
    href: "https://www.gov.br/mds/pt-br/noticias-e-conteudos/desenvolvimento-social/noticias-desenvolvimento-social/tire-suas-duvidas-sobre-o-novo-cadastro-unico"
referencias:
  - label: "AMUNES — novas regras do CadÚnico e Bolsa Família a partir de 2026"
    href: "https://www.amunes.org.br/noticias/3930/cadunico-e-bolsa-familia-tem-novas-regras-a-partir-de-2026"
faq:
  - pergunta: "De quanto em quanto tempo preciso atualizar o CadÚnico?"
    resposta: "No máximo a cada 24 meses (2 anos), mesmo que nada tenha mudado. Mas qualquer mudança de endereço, renda, emprego, nascimento ou óbito exige atualização na hora, sem esperar o prazo."
  - pergunta: "Como sei se meu cadastro está em averiguação ou revisão?"
    resposta: "O aplicativo do Cadastro Único e o app Bolsa Família mostram avisos de 'averiguação cadastral' ou 'revisão cadastral'. Se aparecer, procure o CRAS para atualizar dentro do prazo informado."
  - pergunta: "O que acontece se eu não atualizar?"
    resposta: "O benefício pode ser bloqueado e, se a situação não for regularizada, cancelado. Famílias com cadastro desatualizado há mais de 2 anos são as mais sujeitas a isso."
relacionados:
  - "como-se-inscrever-cadastro-unico"
  - "bolsa-familia-bloqueado-suspenso-cancelado"
---

Manter o Bolsa Família não é só entrar no programa: é manter o seu Cadastro Único em dia. A maioria dos bloqueios começa por um cadastro desatualizado — e isso é totalmente evitável. Veja quando e como atualizar.

## Quando atualizar

Há duas situações que pedem atualização:

- **A qualquer momento**, sempre que mudar algo importante: endereço, renda, emprego de alguém da casa, nascimento de criança, óbito, ou alguém entrando/saindo da família.
- **Pelo menos a cada 24 meses**, mesmo que nada tenha mudado. Esse prazo de 2 anos é a regra geral do cadastro (definida pela Lei nº 15.077/2024 e regulamentada para 2026).

Ou seja: não espere "dar problema" para atualizar. Mudou alguma coisa, atualize.

## O que é a averiguação cadastral

De tempos em tempos, o governo cruza os dados do CadÚnico com outras bases (como emprego formal e renda) e convoca famílias para confirmar as informações. Isso aparece como **"averiguação cadastral"** ou **"revisão cadastral"** nos aplicativos. Em 2026, uma grande revisão (a REV26) convocou milhões de famílias — e o cadastramento **biométrico** passa a ser exigido a partir de novembro de 2026.

Se você receber esse aviso, **não ignore**: procure o CRAS dentro do prazo indicado para não perder o benefício.

## Como atualizar (passo a passo)

A atualização é **gratuita** e parecida com a inscrição:

1. Veja se há aviso de pendência no app do Cadastro Único ou no app Bolsa Família.
2. **Agende e vá ao CRAS** (em muitas cidades o agendamento é obrigatório).
3. Leve documentos de **todos** da família e os comprovantes do que mudou (novo endereço, novo comprovante de renda, certidão de nascimento etc.). O [checklist de documentos](/ferramentas/checklist) ajuda.
4. O responsável familiar confirma e corrige as informações com o atendente.

## O que acontece se você não atualizar

Ficar mais de 2 anos sem atualizar — ou ignorar uma convocação de averiguação — pode levar ao **bloqueio** e, se nada for resolvido, ao **cancelamento** do Bolsa Família e de outros programas ligados ao CadÚnico, como Tarifa Social e Auxílio Gás.

Se o seu benefício já foi bloqueado, ainda dá para resolver: veja o passo a passo em [Bolsa Família bloqueado, suspenso ou cancelado](/bolsa-familia/bolsa-familia-bloqueado-suspenso-cancelado).
